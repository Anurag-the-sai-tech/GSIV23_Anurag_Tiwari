...
import { MoviesData, getUpcomingMoviesApi } from '../../api/getUpcomingMoviesApi';
...
export const getUpcomingMovies = createAsyncThunk('movies/getUpcoming', async (page: number) => {
  ...
  const response = await getUpcomingMoviesApi(pageNumber);
  return response;
});
...
export const getSearchResults = createAsyncThunk('movies/search', async (page: number, thunkAPI) => {
  ...
  const response = await searchMoviesApi(state.movies.searchText, pageNumber);
  return response;
});
